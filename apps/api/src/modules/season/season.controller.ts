...
import { CreateSeasonDto, SeasonService } from './season.service.js';
...
export class SeasonController {
  ...
  @UseGuards(AuthGuard, AccountStatusGuard, PermissionsGuard)
  @RequirePermission(PermissionName.MANAGE_SEASONS)
  createDivision(@Param('id') id: string, @Body() body: any, @Req() req: any) {
    const actor = { id: req.user?.id, role: req.user?.roles?.[0], correlationId: req.id };
  ...
  @UseGuards(AuthGuard, AccountStatusGuard, PermissionsGuard)
  @RequirePermission(PermissionName.MANAGE_SEASONS)
  updateDivision(@Param('divisionId') divisionId: string, @Body() body: any, @Req() req: any) {
    const actor = { id: req.user?.id, role: req.user?.roles?.[0], correlationId: req.id };
