...
export class ClubService {
  ...
  async updateClubStatus(clubId: string, status: string, actorId: string, actorRole: string) {
    ...
    const allowed: Record<$Enums.ClubStatus, $Enums.ClubStatus[]> = {
    ...
    const nextStatus = status as $Enums.ClubStatus;
    if (!allowed[club.status as $Enums.ClubStatus]?.includes(nextStatus)) {
